...
- huskyCI API token authentication
...
### huskyCI API Authentication

The CLI authenticates with the huskyCI API using huskyCI-generated tokens (not GitHub tokens). These tokens are repository-specific and grant access to analyze specific repositories.

### Authentication Methods

The CLI supports two methods for providing authentication tokens:

1. **Environment Variables** (Recommended):
   ```bash
   export HUSKYCI_CLIENT_TOKEN="your-huskyci-token"
   export HUSKYCI_CLIENT_API_ADDR="https://api.huskyci.example.com"
   ```

2. **Configuration File**: Tokens can be configured per target in the config file

### Generating huskyCI API Tokens

Tokens are generated via the huskyCI API endpoint:

```bash
curl -X POST https://api.huskyci.example.com/api/1.0/token \
  -u username:password \
  -H "Content-Type: application/json" \
  -d '{"repositoryURL": "https://github.com/user/repo.git"}'
```

The API returns a `huskytoken` that should be used as `HUSKYCI_CLIENT_TOKEN`.

### Token Characteristics

- **Repository-Specific**: Each token grants access to a specific repository URL
- **API-Generated**: Tokens are generated by the huskyCI API, not GitHub
- **Stored Securely**: Tokens should be kept secure and not shared
...
4. **Authentication Errors**:
   ...
   - Token doesn't match repository URL
...
# 2. Set authentication token (generate via API first)
export HUSKYCI_CLIENT_TOKEN="your-huskyci-token"
...
#### 2. "No authentication token configured"

**Problem**: huskyCI API token not set.
...
2. Set as environment variable:
   ```bash
   export HUSKYCI_CLIENT_TOKEN="generated-token"
   ```
...
#### 5. "Authentication failed" or "Permission denied"

**Problem**: Invalid or missing huskyCI API token.

**Solution**:
- Verify `HUSKYCI_CLIENT_TOKEN` is set correctly
- Ensure token matches the repository URL being analyzed
- Generate a new token if needed via the API
...
**Solution**:
- Verify `HUSKYCI_CLIENT_TOKEN` environment variable is set
- Check config file for token configuration
- Ensure token format is correct (base64 encoded UUID:random)
- Regenerate token if needed via API
...
- **Token Storage**: Environment variable `HUSKYCI_CLIENT_TOKEN` or config file (not `.huskyci` file)
...
- Tokens are repository-specific (one token per repository URL)
